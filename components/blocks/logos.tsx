    ...
    <Section background={data.background} navigationLabel={data.navigationLabel}>
      <div className={`relative w-full max-w-site-full mx-auto ${style?.padding} ${style?.alignment}`}>
        ...
        <div className={`mx-auto ${style.gridWidth}`}>
          <div className={`flex flex-wrap items-center justify-between bg-white p-10 pb-0 -mx-4 -mt-4 ${logoJustifyMobile} sm:-mx-${logoSpacingMobile} sm:-mt-${logoSpacingMobile}`}>
            {data.items && (
              data.items.map(function (item, index) {
                return (
                  <div className={`basis-0 p-4 mb-10 sm:p-${logoSpacingMobile} ${logoWidthMobile}`} key={index}>
                    <img alt={item.alt} src={item.src} className="h-8 w-auto" />
                  </div>
                )
              })
            )}
